...
def subset(paths, samples):
    '''filter the paths according to samples'''
    samples = set(samples)
    filtered_paths = []
...
def log_error_msg(error_list, save_as):
...
def save_dict(dictionary, expt_dir, save_as):
    expt_dir = str(expt_dir)
    if not os.path.exists(expt_dir):
        os.makedirs(expt_dir)
    with open(os.path.join(expt_dir, save_as), 'wb') as f:
        pickle.dump(dictionary, f)
    return

def log_epoch_metrics(epoch_log, metrics, title):
    for x, m in zip(title, metrics):
